use crate::earth::{Carnivore, Herbivore, CARNIVORE_REACHABLE_RANGE};
use amethyst::{
...
impl<'s> System<'s> for HerbivoreCarnivore {
    ...
    fn run(&mut self, (entities, mut herbivores, mut carnivores, transforms): Self::SystemData) {
        'herbivore_loop: for (herbivore_entity, herbivore, herbivore_transform) in
            (&*entities, &mut herbivores, &transforms).join()
        {
            for (_, carnivore, carnivore_transform) in
                (&*entities, &mut carnivores, &transforms).join()
            {
                ...
                if x_diff * x_diff + y_diff * y_diff
                    < CARNIVORE_REACHABLE_RANGE * CARNIVORE_REACHABLE_RANGE
                {
                    carnivore.nutrition += herbivore.nutrition;
                    entities
                        .delete(herbivore_entity)
