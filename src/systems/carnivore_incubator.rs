...
use crate::earth::{Acceleration, Velocity, Carnivore, CARNIVORE_MAX_SPEED, CARNIVORE_MIN_LIFE, CARNIVORE_MAX_LIFE, CARNIVORE_INITIAL_NUTRITION};
...
impl<'s> System<'s> for CarnivoreIncubator {
    ...
    fn run(
        ...
        for (entity, carnivore, transform, sprite_render) in (&*entities, &mut carnivores, &transforms, &sprite_renders).join() {

            if carnivore.life <= 0 {
                entities.delete(entity).expect("Failed to delete a carnivore.");
        ...
        for (transform, sprite_render) in new_plants {
            entities
                ...
                .with(Acceleration{max_speed: CARNIVORE_MAX_SPEED, x: 0.0, y: 0.0}, &mut accelerations)
                .with(
                    Carnivore{
