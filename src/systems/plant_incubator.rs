...
use crate::earth::{Velocity, Acceleration, Plant, PLANT_MAX_SPEED, PLANT_MIN_LIFE, PLANT_MAX_LIFE, PLANT_INITIAL_NUTRITION, PLANT_BOID_SEPARATION_DISTANCE};
            ...
            if plant.life <= 0 {
                ...
                if plant.nutrition >= PLANT_INITIAL_NUTRITION * 2 {
                    let difference = {
                        ...
                        let s = PLANT_BOID_SEPARATION_DISTANCE * (rng.gen::<f32>().sqrt());
                        (s * r.cos(), s * r.sin())
                    };
